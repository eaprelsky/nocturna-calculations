...
# $([ "$target_instance" = "blue" ] && echo "Blue" || echo "Green") instance (currently active)
server 127.0.0.1:$target_port;

# $([ "$target_instance" = "blue" ] && echo "Green" || echo "Blue") instance (inactive)
...
reload_nginx() {
    ...
    # Try docker-compose nginx reload (if nginx runs in docker)
    if docker ps --format '{{.Names}}' | grep -q nginx; then
        docker exec nginx nginx -s reload 2>/dev/null || true
        log_success "Nginx (docker) reloaded successfully"
        return 0
    fi
...
main() {
    ...
    if check_health "$target_instance"; then
        ...
        log_info ""
        log_info "Status:"
        log_info "  Active: $target_instance (port $([ "$target_instance" = "blue" ] && echo "8081" || echo "8082"))"
        log_info "  Inactive: $current_instance (port $([ "$current_instance" = "blue" ] && echo "8081" || echo "8082"))"
        log_info ""
...
if [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ -z "$1" ]; then
    ...
    echo ""
    echo "Arguments:"
    echo "  blue   - Switch to blue instance (port 8081)"
    echo "  green  - Switch to green instance (port 8082)"
    echo ""
    echo "Examples:"
