...
echo -e "${BLUE}=== Blue-Green Infrastructure Check ===${NC}\n"
...
# Check Network
echo -e "${BLUE}4. Docker Network${NC}"
...
# Check Volume
echo -e "${BLUE}5. PostgreSQL Volume${NC}"
if docker volume inspect nocturna_postgres_data >/dev/null 2>&1; then
    echo -e "   ${GREEN}✓${NC} Volume 'nocturna_postgres_data' exists"
    
    # Show size
    size=$(docker volume inspect nocturna_postgres_data --format='{{.Mountpoint}}' 2>/dev/null)
    if [ -n "$size" ]; then
        echo -e "   ${BLUE}ℹ${NC} Mountpoint: $size"
    fi
else
...
# Check Blue Instance
echo -e "${BLUE}6. Blue Instance${NC}"
if docker ps --format '{{.Names}}' | grep -q "^nocturna-bot-blue$"; then
    echo -e "   ${GREEN}✓${NC} Container is running"
...
echo -e "${BLUE}7. Green Instance${NC}"
if docker ps --format '{{.Names}}' | grep -q "^nocturna-bot-green$"; then
    echo -e "   ${GREEN}✓${NC} Container is running"
...
blue_running=$(docker ps --format '{{.Names}}' | grep -c "^nocturna-bot-blue$" || echo 0)
green_running=$(docker ps --format '{{.Names}}' | grep -c "^nocturna-bot-green$" || echo 0)

if [ "$blue_running" -eq 0 ] && [ "$green_running" -eq 0 ]; then
    echo -e "${YELLOW}⚠${NC} No bot instances running"
fi
